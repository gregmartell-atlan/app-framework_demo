...
- Option C: Atlan App Framework task (app.connector.GitHubConnector.sync_glossary)
...
async def _clone_wiki(repo_full_name: str, github_token: str, clone_root: Optional[Path] = None) -> list[WikiPageRecord]:
    """Clone the GitHub wiki for repo_full_name and return WikiPageRecord list.

    If clone_root is provided, clones there (used by tests); otherwise uses a tempdir.
    """
    ...
    if clone_root is not None:
        ...
        pages = await asyncio.to_thread(_do_clone, clone_root)
    else:
        with tempfile.TemporaryDirectory() as tmp:
            target = Path(tmp) / "wiki"
            pages = await asyncio.to_thread(_do_clone, target)
    return pages

...
Option B in the three-option deployment matrix. Business logic lives in
app/glossary_sync.py — this script parses CLI flags / env vars, builds the
GlossarySyncConfig + AtlanClient, runs the sync, and prints the result.

Env vars:
  ATLAN_BASE_URL   default: https://dsm.atlan.com
  ATLAN_API_KEY    required
  GITHUB_TOKEN     required (used to clone the wiki)
  GITHUB_REPO      default: sony-telemetry/schema-registry
  GLOSSARY_NAME    default: sony_telemetry
...
async def _amain() -> int:
    ...
    repo = os.environ.get("GITHUB_REPO", "sony-telemetry/schema-registry")
    glossary_name = os.environ.get("GLOSSARY_NAME", "sony_telemetry")

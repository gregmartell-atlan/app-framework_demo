"""Mapping functions from GitHub wiki records to Atlan glossary assets.

Produces AtlasGlossary / AtlasGlossaryCategory / AtlasGlossaryTerm objects
using the pyatlan_v9 SDK's creator() helpers.

Hierarchy:
  AtlasGlossary          — one per sync target (e.g. "sony_telemetry")
  AtlasGlossaryCategory  — top-level: Client / Native / Tooling / Shared Schemas
  AtlasGlossaryCategory  — sub-level: Events / Event Templates
  AtlasGlossaryTerm      — one per wiki page
"""
...
def _parse_extends_includes(content: str) -> tuple[list[str], list[str]]:
    """Return (extends_slugs, includes_slugs) from the Structure section."""
...
def map_glossary(name: str) -> AtlasGlossary:
    g = AtlasGlossary.creator(name=name)
    g.description = "Telemetry schema terms synced from the GitHub wiki."
    return g


def map_glossary_category(section_name: str, glossary_qn: str) -> AtlasGlossaryCategory:
    """Create a top-level glossary category."""
...
def build_relationship_updates(
    ...
    """Return a minimal term update setting is_a (Extends) and user_def_relationship_to (Includes).

    Extends → is_a (single parent, first resolved slug wins)
    Includes → user_def_relationship_to with APPEND semantic
    Returns None if no resolvable cross-references.
    """
